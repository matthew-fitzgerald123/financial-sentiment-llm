...
- **Infra**: Docker (build + smoke-test gated in CI, container-native `HEALTHCHECK` on `/health`) · AWS ECS EC2 (g4dn.xlarge, NVIDIA T4, container-level ECS health check independent of the ALB) · Terraform (fmt + validate gated in CI) · GitHub Actions
