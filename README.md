...
- **Infra**: Docker (build + smoke-test gated in CI, container-native `HEALTHCHECK` on `/health` in both the ECS image and the Cloud Run GGUF image) · AWS ECS EC2 (g4dn.xlarge, NVIDIA T4, container-level ECS health check independent of the ALB) · Terraform (fmt + validate gated in CI) · GitHub Actions
